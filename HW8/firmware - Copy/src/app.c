                ...
                switch(appData.receiveDataBuffer[0])
                {
                    ...
                    case 0x2:
                        ...
                        if(appData.hidDataTransmitted)
                        {
                            ...
                            int accel_divisor=max_accel_reading/100;
                            int i=1;
                            /* Echo back to the host PC the command we are fulfilling in
                             * the first byte.  In this case, the Get Push-button State
                             * command. */ //not push button this time

                            appData.transmitDataBuffer[0] = 0x2;
                            

                            //transmit packet created here

                            while (appData.hidDataTransmitted){
                            ...
                            if (_CP0_GET_COUNT()<200000){
                                ...
                                while (i<=3){
                                    ...
                                    double data=(double)abs(accels[i-1])/(double)accel_divisor;
                                    appData.transmitDataBuffer[i*2]=is_negative;
                                    //appData.transmitDataBuffer[(i*2)+1]=(double)(abs(accels[i-1]))/accel_divisor;
                                    appData.transmitDataBuffer[(i*2)+1]=data;
                    ...
                    case 0x3:
                        ...
                        while (j<25){
                            message[j]=appData.receiveDataBuffer[j+2];
                            j++;
                        }

                        display_string(message, appData.receiveDataBuffer[1], 0);
