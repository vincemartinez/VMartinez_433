...
void APP_Tasks ( void )
{
    ...
    short accels[3];
    long sensitivity = 6000; //higher is less sensitive
    double x_weight;
    double y_weight;

    //int8_t dir_table[] ={-4,-4,-4, 0, 4, 4, 4, 0}; // circles
        ...
        case APP_STATE_MOUSE_EMULATE:
            ...
            if(appData.emulateMouse)
            {
                ...
                if(movement_length > 50)
                {
                    ...
                    x_weight=acc_mouse_x(accels[0],sensitivity);
                    y_weight=acc_mouse_y(accels[1],sensitivity);
                    appData.mouseButton[0] = MOUSE_BUTTON_STATE_RELEASED;
                    appData.mouseButton[1] = MOUSE_BUTTON_STATE_RELEASED;
                    //appData.xCoordinate =(int8_t)dir_table[vector & 0x07] ;
                    //appData.yCoordinate =(int8_t)dir_table[(vector+2) & 0x07];
                    appData.xCoordinate = (double) x_weight;
                    appData.yCoordinate = (double) y_weight;
                    //vector ++;
                    movement_length = 0;
                }
            }
